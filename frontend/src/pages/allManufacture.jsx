...
// ASSUMPTION: Backend provides endpoints similar to purchases:
// GET alltransaction/manufacturetransaction/branch/:branchId/
// POST alltransaction/manufacturetransaction/
// Response structure assumed analogous to purchase transactions:
//   id, date, bill_no, total_amount, manufacture: [{ product_name, quantity, unit_cost, total_cost }]
// Adjust keys in mapping if backend differs.
...
export default function AllManufactureTransactions() {
  const api = useAxios()
  ...
  const fetchInitData = async () => {
    try {
      ...
      console.error(err)
      setError('Failed to fetch initial data (endpoint may not exist yet)')
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <div className='flex min-h-screen bg-gradient-to-br from-slate-900 to-slate-800'>
      ...
      <div className='flex-grow p-4 px-8 lg:p-6 lg:ml-64'>
        ...
        >
          <h1 className='text-3xl lg:text-4xl font-bold text-white mb-4 lg:mb-0'>Manufacture Transactions</h1>
          <Button
            onClick={() => navigate('/')}
            variant='outline'
        ...
        <div className='space-y-6'>
          {transactions.length > 0 ? (
            transactions.map((transaction) => (
              <Card key={`${transaction.id}-${transaction.date}`} className='bg-gradient-to-b from-slate-800 to-slate-900 border-none shadow-lg hover:shadow-xl transition-shadow duration-300'>
            ...
            <div className='text-center text-white'>No manufacture transactions found.</div>
          )}
        </div>
      ...
      <Button
        ...
        onClick={() => navigate(`/manufacture/form/branch/${branchId}`)}
      >
