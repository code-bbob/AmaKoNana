...
import { Printer, Download, ChevronDown, Calendar, ArrowLeft, CheckCircle } from "lucide-react"
...
const AllIncomeExpenseReport = () => {
	...
	const handleApprove = async () => {
		...
		try {
			await api.post('alltransaction/closing-cash/', { branch: parseInt(branchId, 10), amount: data.net_cash_in_hand, date: format(new Date(), 'yyyy-MM-dd') })
				...
				<CardHeader className="border-b border-slate-700 print:border-gray-200 relative">
					...
					{role === 'Admin' && (
						<div className=" flex items-center space-x-2 print:hidden">
							<Button onClick={() => setConfirmOpen(true)} disabled={approveStatus === 'Approved'} variant="outline" className="bg-emerald-600 hover:bg-emerald-700 text-white border-none disabled:opacity-50">
								<CheckCircle className="mr-2 h-4 w-4" /> {approveStatus ? approveStatus : 'Approve'}
							</Button>
						</div>
					)}
				...
				<CardContent className="pt-4 sm:pt-6">
					...
					<Table className="min-w-[640px]">
						...
						<TableBody>
							{data.transactions.map((t,i)=>(
								<TableRow key={i} onClick={() => navigate(t.type === 'Sale' ? `/sales/branch/${branchId}/editform/${t.id}` : t.type === 'Order' ? `/orders/branch/${branchId}/editform/${t.id}` : t.type === 'Withdrawal' ? `/withdrawals/branch/${branchId}/edit/${t.id}` : `/expenses/branch/${branchId}/edit/${t.id}`)} className="cursor-pointer hover:bg-slate-700 print:hover:bg-transparent">
					...
					<div className="mt-6 grid gap-4 sm:grid-cols-2 lg:flex lg:justify-end">
						<div className="w-full sm:w-80 bg-slate-800 p-4 rounded-lg print:bg-gray-100">
							...
							<div className="flex justify-between mb-2 border-t border-slate-600 pt-2"></div><div className="flex justify-between mb-2"><span className="font-semibold text-white print:text-black">Cash Income :</span><span className="text-white print:text-black">{data.totals.cash.toLocaleString('en-US',{style:'currency',currency:'NPR'})}</span></div>
							<div className="flex justify-between mb-2"><span className="font-semibold text-white print:text-black">Cash Expense:</span><span className="text-white print:text-black">{data.totals.cash_expense.toLocaleString('en-US',{style:'currency',currency:'NPR'})}</span></div>
			...
			<Dialog open={confirmOpen} onOpenChange={setConfirmOpen}>
				<DialogContent>
					<DialogHeader>
						...
						<DialogDescription>
							Are you sure you want to verify and store today's closing cash of {data?.net_cash_in_hand?.toLocaleString('en-US',{style:'currency',currency:'NPR'})}? This action records the amount for today.
						</DialogDescription>
					</DialogHeader>
					<DialogFooter className="flex justify-end space-x-3">
						<Button variant="outline" onClick={() => setConfirmOpen(false)} className="border-slate-600 text-black hover:bg-slate-700">Cancel</Button>
						<Button onClick={handleApprove} className="bg-emerald-600 hover:bg-emerald-700 text-white">Confirm</Button>
					</DialogFooter>
				</DialogContent>
			</Dialog>
		</div>
