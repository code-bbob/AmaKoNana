...
const AllWithdrawalsReport = () => {
  ...
  const fetchData = async (params = {}) => {
    ...
    try {
      ...
      const response = await api.get(`alltransaction/withdrawals-report/branch/${branchId}/?${queryString}`);
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 to-slate-800 p-4 lg:px-8 print:bg-white print:p-0">
      ...
      <Card className="bg-gradient-to-b from-slate-800 to-slate-900 border-none shadow-lg print:shadow-none print:bg-white">
        <CardHeader className="border-b border-slate-700 print:border-gray-200">
          <CardTitle className="text-2xl lg:text-3xl font-bold text-white print:text-black">Withdrawals Report</CardTitle>
            ...
            <TableBody>
              {data.items.map((item, idx) => (
                <TableRow key={idx}>
                  ...
                  <TableCell className="text-white print:text-black">{item.person_name || '—'}</TableCell>
          ...
          <div className="mt-6 flex justify-end">
            <div className="w-72 bg-slate-800 p-4 rounded-lg print:bg-gray-100">
              <div className="flex justify-between text-sm mb-2">
                <span className="font-semibold text-white print:text-black">Transactions:</span>
                <span className="text-white print:text-black">{data.count}</span>
              </div>
              <div className="flex justify-between mb-2">
                <span className="font-semibold text-white print:text-black">Total Withdrawals:</span>
                <span className="text-white print:text-black">{data?.total_withdrawals?.toLocaleString("en-US")}</span>
              </div>
            </div>
          </div>
...
export default AllWithdrawalsReport;
